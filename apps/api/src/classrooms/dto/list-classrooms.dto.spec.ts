...
import { CLASSROOMS_PAGE_SIZE_MAX, EnglishLevel, InstructionMode } from '@academia/types';
...
describe('ListClassroomsDto (validación)', () => {
  ...
  it('rechaza un modo de instrucción que no es de la lista', async () => {
    expect(await camposInvalidos({ instructionMode: 'TELEPATIA' })).toContain('instructionMode');
  });

  it('acepta un modo de instrucción válido, solo', async () => {
    expect(await camposInvalidos({ instructionMode: InstructionMode.INTERPRETE_LSC })).toHaveLength(
      0,
    );
  });
