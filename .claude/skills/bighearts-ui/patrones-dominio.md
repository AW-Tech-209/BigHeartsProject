...
- **Modo de instrucción, en su propia línea (HU-507):** `<ModoInstruccion>` (`components/dominio/`) va bajo el título, **nunca en una fila con los apoyos** y nunca colapsa. Tono `info` + ícono + texto (`Hand` LSC nativa, `Languages` con intérprete); sin declarar, `muted` con borde punteado, `CircleHelp` y «Modo de instrucción sin declarar». `Coincide con tu preferencia` va en esa misma línea, porque la coincidencia es sobre el modo. En el detalle, `tamano="destacado"`.
- **Fila de estado y apoyos (HU-414, HU-507):** `<EstadoAula>` y `Tu clase` van siempre visibles. Los apoyos (`ClassroomSupport`) forman una lista aparte (`aria-label="Apoyos de la clase"`, chips `neutral` en `text-xs`) y son lo único que colapsa tras un `<button aria-expanded>` `+N`. El hueco es un techo (`maxEtiquetasVisibles`, prop, default 2) menos `Tu clase` si está, para que la fila no envuelva; el `+N` despliega el resto en una banda `También:` al pie del renglón. La acción vive en la zona 4 y su mensaje asociado (cuenta atrás de acceso, «ya no se puede cancelar», error de reserva) se reubica en la banda de aviso, también al pie.
