...
3. Render detecta [render.yaml](render.yaml) y propone el servicio `academia-api-staging`. Aplícalo.
...
Cuando termine, anota la URL pública: `https://academia-api-staging.onrender.com` (o la que asigne).
...
3. Comprobación directa del backend:
   ```bash
   curl https://academia-api-staging.onrender.com/health
   ```
4. Comprobación de CORS (debe devolver la cabecera con tu origen de Vercel):
   ```bash
   curl -s -D - -o /dev/null https://academia-api-staging.onrender.com/health \
