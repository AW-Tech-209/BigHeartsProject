...
>
> **Precisión D40 (2026-09-06).** El corte `proximas` / `pasadas` es **`endsAt`**, no `scheduledAt`:
> una clase en curso sigue en `proximas` hasta que termina. La disyunción y la exhaustividad de D24
> no cambian, solo el instante de la frontera.
...
| `durationMinutes`      | int                                                 |                                                                                                                                                                                                                            |
| `endsAt`               | `timestamptz`                                       | `= scheduledAt + durationMinutes`. La escribe la app en cada `create`/`update` (`finDelAula()`); un **CHECK** garantiza que no diverja, como `current_bookings >= 0`. Existe para que «¿ya terminó?» sea un `WHERE` (D40). |
...
> Las reglas de "ya no se puede tocar" (editar, cancelar) se escriben contra `scheduledAt` —una
> clase que **empieza** deja de ser editable— no contra el estado. En cambio, la **pertenencia a
> lista** (pasado vs. futuro en «Mis clases», «Mis aulas», historial y panel) se corta en `endsAt`:
> una clase en curso todavía no es pasado (D40).
...
| D34 | El pasado **se muda a `/historial`**: `/mis-clases` y `/mis-aulas` solo muestran lo que viene                                                                               | HU-404     | Con una pantalla de historial propia, dejar los filtros `pasadas` donde estaban daría dos pantallas listando lo mismo. **Ajusta D24**: su semántica de grupos disjuntos se conserva, pero vive en el historial. «Pasado» = clase **terminada** (`endsAt`), no empezada (D40).                                                                                                             |
...
| D40 | Columna `Classroom.endsAt` (`= scheduledAt + durationMinutes`, con `CHECK`); el corte pasado/futuro de «Mis clases», «Mis aulas», historial y panel se hace contra `endsAt` | 2026-09-06 | Una clase en curso desaparecía de «Mis clases» / «Mis aulas» apenas empezaba y se colaba en el historial. El corte fino («¿ya terminó?») es una suma de dos columnas que Prisma no filtra; en vez de resolverlo en memoria por pantalla, se persiste. La editabilidad y el catálogo público siguen cortando en `scheduledAt` (una clase que **empieza** deja de editarse y de ofrecerse). |
