...
- [x] **T1** — En `packages/types`: filtros de «mis reservas» y el tipo de respuesta. **Reutiliza
      el item de listado de aula**; no declares un tipo paralelo. Luego `npm run build:types`.

### Backend

- [x] **T2** — `GET /bookings/mias` con `@Roles('STUDENT')`, acotado **al token**. Devuelve las
      reservas del estudiante con los datos del aula y su `myBookingStatus`.
- [x] **T3** — Filtros `proximas` · `pasadas` · `canceladas` · `todas`, con la semántica de D24.
      `todas` se sirve como dos listas concatenadas: próximas ascendente, historial descendente.
- [x] **T4** — El `meetingLink` **no viaja** aquí. §4.8 regla 2, sin excepción: es un listado.
- [x] **T5** — Tests: el alcance no se puede ampliar con ningún parámetro; los tres filtros suman
      el total y ninguna reserva aparece dos veces; `meetingLink` ausente.

### Frontend

- [x] **T6** — Rellenar `/mis-clases`: tarjetas con estado, fecha con zona explícita, profesor y
      modos de comunicación. Los 4 estados. El vacío dice qué hacer y lleva al catálogo.

### Documentación

- [x] **T7** — Retirar la nota de «pendiente de Sprint 3» que D18 dejó sobre esta ruta.

## ✅ Criterios de aceptación

- [x] **AC1** — Un estudiante con reservas ve **sus** clases en `/mis-clases`, ordenadas por
      cercanía, con la próxima primero.
- [x] **AC2** — Los tres filtros son **disjuntos y exhaustivos**: suman el total y ninguna reserva
      sale en dos. Una clase cancelada del mes que viene aparece en `canceladas`, no en `proximas`.
- [x] **AC3** — **Autorización:** el alcance sale del token. No hay parámetro que devuelva las
      reservas de otro, y un `TEACHER` o `ADMIN` recibe `403`.
- [x] **AC4** — **El `meetingLink` no aparece** en la respuesta. Verificado con un test.
- [x] **AC5** — Sin reservas, la pantalla explica cómo conseguir una y enlaza al catálogo. No se
