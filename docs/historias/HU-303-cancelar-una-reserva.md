...
- [ ] **T1** — En `packages/types`: códigos `BOOKING_WINDOW_CLOSED`, `BOOKING_NOT_FOUND`,
      `BOOKING_ALREADY_CANCELLED`, y el campo que dice si la reserva **todavía se puede cancelar**
      —para que el frontend pinte sin recalcular la regla—. Luego `npm run build:types`.

### Backend

- [ ] **T2** — `CANCELLATION_WINDOW_MINUTES` en `config/env.schema.ts`, por defecto 60.
- [ ] **T3** — `POST /bookings/:id/cancelar` con `@Roles('STUDENT')`: valida **propiedad de la
      reserva** y ventana, y en **una sola transacción** marca `CANCELLED`, escribe `cancelledAt` y
      decrementa `currentBookings`.
- [ ] **T4** — Emitir `BOOKING_CANCELLED` por el puerto (D29).
- [ ] **T5** — Tests: cancelar la reserva de otro → `404` (no `403`: no se confirma que exista);
      fuera de ventana → `BOOKING_WINDOW_CLOSED`; el contador baja exactamente uno; cancelar dos
      veces no lo baja dos veces; y **el cupo liberado se puede volver a reservar**.

### Frontend

- [ ] **T6** — Acción de cancelar en «Mis reservas» y en el detalle, con confirmación accesible que
      diga **qué clase** se cancela. Sin optimismo: el estado cambia cuando responde el servidor.
- [ ] **T7** — Pasada la ventana, la acción **no se ofrece** y en su lugar se explica por qué, con
      la hora límite en la zona del usuario. Nunca un botón muerto sin explicación.

## ✅ Criterios de aceptación

- [ ] **AC1** — Dentro de la ventana, cancelar deja la reserva en `CANCELLED` con `cancelledAt`, y
      `currentBookings` baja **exactamente uno**. La fila no se borra.
- [ ] **AC2** — El cupo liberado queda **inmediatamente** disponible: otro estudiante puede
      reservarlo acto seguido. Verificado con un test.
- [ ] **AC3** — A 59 minutos del inicio la cancelación responde `BOOKING_WINDOW_CLOSED`; a 61
      minutos, `200`. La decisión es del servidor.
- [ ] **AC4** — **Autorización:** cancelar la reserva de otro estudiante responde `404`, sin
      revelar que existe.
- [ ] **AC5** — Cancelar dos veces la misma reserva **no** decrementa el contador dos veces.
